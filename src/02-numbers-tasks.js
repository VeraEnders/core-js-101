...
const getAngleBetweenVectors = (x1, y1, x2, y2) => {
  ...
  const magnitudeOfVectorX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magnitudeOfVectorY = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosine = dotProduct / (magnitudeOfVectorX * magnitudeOfVectorY);
...
const roundToPowerOfTen = (num, pow) => {
  const tens = 10 ** pow;
  const intRounded = Math.round(num / tens);
  return intRounded * tens;
};
...
const toNumber = (value, def) => {
  ...
  if (typeof value === 'undefined' || value === null || Number.isNaN(valueNum)) {
    return def;
  }
  return valueNum;
};
