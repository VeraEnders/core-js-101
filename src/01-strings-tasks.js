...
const extractNameFromTemplate = (str) => str.substring(str.indexOf(',') + 2, str.indexOf('!'));
...
const getRectangleString = (width, height) => {
  const firstLineShort = '┌┐\n';
  const firstLineLong = `┌${'─'.repeat(width - 2)}┐\n`;
  const lastLineShort = '└┘\n';
  const lastLineLong = `└${'─'.repeat(width - 2)}┘\n`;
  const middleLine = `│${' '.repeat(width - 2)}│\n`;

  let result = '';
  if (height === 2 && width === 2) {
    result = firstLineShort + lastLineShort;
  } else if (height === 2 && width > 2) {
    result = firstLineLong + lastLineLong;
  } else {
    result = firstLineLong + middleLine.repeat(height - 2) + lastLineLong;
  }
  return result;
};
